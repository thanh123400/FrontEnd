      ...
      <div className="Book_infor_setting">
        <div className="Book_infor_content">
          <div className="Book_infor_content_setting">
            ...
            {Library.map((slide) => {
                  if (slide.pk === numberIdbook) {
                    return <img className="p1" src={`http://35.212.202.252:8000` + slide.fields.image} />
                  } else {
                    return null;
                  }
                })}
              ...
              <button>
                {Library.map((slide, index) => {
                  if (slide.pk === numberIdbook) {
                    return <Link to={`/ReadBook/${slide.pk}`}>Read Now</Link>;
                  } else {
                    return null;
                  }
                })}
            ...
            <div className="Book_content_2">
              {Library.map((slide, index) => (
                ...
                >
                  {slide.pk === numberIdbook ? slide.fields.title : null}
                </h1>
              ))}
              ...
              >
                Author by: {data_star.name_author}
              </div>
              <div>
                <StarRating
              ...
              {Library.map((slide, index) => (
                <div key={index}>
                  {slide.pk === numberIdbook ? (
                    <div>
                      <span>
                        {slide.fields.Description.slice(0, 300)}
                        {showMore ? (
                          <span>
                            {slide.fields.Description.slice(300)}
                            <a
