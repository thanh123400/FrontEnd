      ...
      <div className="content">
        <div className="slider-wrapper-1">
          ...
          <Slider {...Slide_setting}>
            {Library.map((slide, index) => ( slide.model === "appModels.book" &&
              <div className="img-customize" key={index} >
                <div >
                <Link onClick={() =><GoToBook idBook={slide.pk} />} style={{ cursor: 'pointer', userSelect: 'none' }} 
                to={`/bookInfor/${slide.pk}`}>
                <img src={slide.fields.image.replace("/view?usp=share_link", "/uc?export=view") }/>
        </Link>
